// 脚本在HTML文档加载和解析完成后立即执行，负责动画等即时效果
...
function typeWriterForElement(element, text, callback, i = 0, speed = 15) {
    if (element.closest('.flip-card-container').classList.contains('is-active') && i < text.length) {
        element.innerHTML = text.substring(0, i + 1) + '<span class="typing-cursor">|</span>';
        setTimeout(() => typeWriterForElement(element, text, callback, i + 1, speed), speed);
    } else if (!element.closest('.flip-card-container').classList.contains('is-active')) {
        return;
    } else {
...
function typeParagraphsSequentially(paragraphs, originalTexts, index = 0) {
    if (index >= paragraphs.length || !paragraphs[index].closest('.flip-card-container').classList.contains('is-active')) {
        return;
    }
